...
public class RegisterTask extends AsyncTask<String,Void,String> {
    ...
    public RegisterTask(DownloadCallback downloadCallback) {
        try {
    ...
    protected String doInBackground(String... strings) {
        try {
            ...
            try {
                HttpPost request = new HttpPost(APIUrl.BASE_URL_2+"register");
                JSONObject jObj = new JSONObject();
                jObj.put("name", strings[0]);
                jObj.put("roll_no", strings[1]);
                jObj.put("mobile", strings[2]);
                jObj.put("email", strings[3]);
                jObj.put("parents",strings[4]);
                jObj.put("school_id",strings[5]);
                jObj.put("class_id",strings[6]);
                System.out.println("onApi Reg "+ jObj.toString());
