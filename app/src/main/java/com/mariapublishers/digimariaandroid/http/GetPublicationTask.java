 ...
 */
public class GetPublicationTask extends AsyncTask<String,Void,String> {
    private DownloadCallback callback;

    public GetPublicationTask(DownloadCallback downloadCallback) {
        try {
    ...
    protected String doInBackground(String... strings) {
        try {
            ...
            try {
                HttpPost request = new HttpPost(APIUrl.BASE_URL_2 + "ourpublication");
                request.setHeader("x-token-id", strings[2]);
                JSONObject jObj = new JSONObject();
                jObj.put("student_id", strings[0]);
                jObj.put("view_type", strings[1]);
