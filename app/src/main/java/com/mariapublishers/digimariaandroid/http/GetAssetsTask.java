 ...
 */
public class GetAssetsTask extends AsyncTask<String,Void,String> {
    private DownloadCallback callback;


    public GetAssetsTask(DownloadCallback downloadCallback) {
        try {
    ...
    protected String doInBackground(String... strings) {
        try {
            ...
            try {
                HttpPost request = new HttpPost(APIUrl.BASE_URL_2+"getassets");
                request.setHeader("x-token-id", strings[2]);
                JSONObject jObj = new JSONObject();
                jObj.put("student_id", strings[0]);
                jObj.put("assets_id", strings[1]);
