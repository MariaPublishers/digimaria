...
public class LoginTask extends AsyncTask<String,Void,String> {
    ...
    public LoginTask(DownloadCallback downloadCallback) {
        try {
    ...
    protected String doInBackground(String... strings) {
        try {
            ...
            try {
                HttpPost request = new HttpPost(APIUrl.BASE_URL_2+"login");
                JSONObject jObj = new JSONObject();
                jObj.put("username", strings[0]);
                jObj.put("password", strings[1]);
                jObj.put("school_code", strings[2]);
                jObj.put("device_token", strings[3]);
                jObj.put("unique_id",strings[4]);
                jObj.put("device_type","1");
